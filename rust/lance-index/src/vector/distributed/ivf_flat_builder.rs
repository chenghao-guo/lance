...
use bytes::Bytes;
// use lance_arrow::FixedSizeListArrayExt; // Unused import
...
async fn wait_for_all_shards(store: &ObjectStore, shard_root: &Path, world: usize, timeout_ms: u64) -> Result<()> {
    ...
    loop {
        ...
        for r in 0..world {
            ...
            let p = per.child("unsorted.lance");
            let exists = store.exists(&p).await?;
            if !exists {
                all_exist = false;
                break;
            }
...
/// - comm: communicator
/// - out_dir: unified output directory to write auxiliary.idx; all ranks pass the same path
///
/// Return: for sequential test support, non-root ranks (rank != 0) perform the merge and return Ok(Some(aux_path));
/// root (rank == 0) writes its shard and returns Ok(None) immediately
pub async fn build_ivf_flat_distributed(
    ...
    // Compute local partition ids using the single-node transformer; handle empty shard gracefully
    let ivf = new_ivf_transformer(centroids.clone(), dist, vec![]);
    let part_ids: UInt32Array = if local_vectors.len() == 0 {
        // empty shard: no assignments
        UInt32Array::from(Vec::<u32>::new())
    } else {
        ivf.compute_partitions(local_vectors)?
    };
    ...
    // Replace vector column with FLAT (for FLAT quantizer, it's identical slice). For empty shards, build an empty FSL with correct dim.
    let vectors_arr: ArrayRef = if local_vectors.len() == 0 {
        use arrow_array::Float32Array as F32Arr;
        Arc::new(FixedSizeListArray::new(
            Arc::new(Field::new("item", DataType::Float32, false)),
            dim as i32,
            Arc::new(F32Arr::from(Vec::<f32>::new())),
            None,
        )) as ArrayRef
    } else {
    ...
    // Root rank: write shard and return immediately; non-root ranks will handle waiting & merge
    if rank == 0 {
        return Ok(None);
    }

    // Non-root waits for all expected shard files to be available, then proceeds to merge
    wait_for_all_shards(&store, &shard_root, world, 10_000).await?;

    // Collect all shard paths deterministically (same directory layout for all ranks)
