...
// Helper to pass Communicator into spawn_blocking without borrowing lifetime issues
#[derive(Copy, Clone)]
#[allow(dead_code)] // This struct is not used yet
struct CommPtr(*const dyn super::communicator::Communicator);
...
impl DistributedIvfCoordinator {
    ...
    pub async fn train_kmeans_allreduce(
        ...
        num_partitions: usize,
        comm: Arc<dyn super::communicator::Communicator>,
    ) -> Result<FixedSizeListArray> {
        ...
        // Deterministic init matching single-worker: mean-of-all + farthest-point selection
        let mut centroids = vec![vec![0f32; dim]; num_partitions];
        if comm.rank() == 0 {
            // First centroid: mean of all local rows
            let inv = 1f32 / local_data.len() as f32;
            for i in 0..local_data.len() {
                let row = &values.values()[i * dim..(i + 1) * dim];
                for d in 0..dim {
                    centroids[0][d] += row[d] * inv;
                }
            }
            // Subsequent centroids: farthest-point from the nearest existing centroid
            for k in 1..num_partitions {
        ...
        // Offload broadcast to blocking thread
        let comm_clone = Arc::clone(&comm);
        centroids = tokio::task::spawn_blocking(move || -> Result<Vec<Vec<f32>>> {
            let mut c = centroids;
            comm_clone.bcast_centroids(&mut c, 0)?;
            Ok(c)
        })
        .await
        .map_err(|e| Error::Index {
            message: format!("spawn_blocking bcast failed: {}", e),
            location: location!(),
        })??;
...
pub struct PartitionAssignment {
    ...
    pub partition_assignments: HashMap<usize, Vec<usize>>, // partition_id -> vector indices
}
